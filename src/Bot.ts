import Eris, { Client, CommandInteraction, Constants, Message } from 'eris';
...
const log: Function = (message: string) =>
  console.log(`[${moment().format('YYYY-MM-DD HH:MM:ss')}] LOG: ${message}`);
...
const loadEvents = (bot: Client) => {
  glob(`dist/events/*.js`).then(async (eventFiles: string[]) => {
    log('⏳ Loading events...');
